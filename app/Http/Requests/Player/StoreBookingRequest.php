...
class StoreBookingRequest extends FormRequest
{
    ...
    {
        $account = auth('player')->user();
        $participantRule = Rule::exists('players', 'id')->where(
            $account?->type === 'parent'
                ? fn ($query) => $query->where('parent_id', $account->id)->where('type', 'player')
                : fn ($query) => $query->where('id', $account?->id)
        );
        ...
        return [
            ...
            'player_ids' => array_filter([
                ...
                $account?->type === 'player' ? 'max:1' : null,
            ]),
            'player_ids.*' => ['integer', 'distinct', $participantRule],
